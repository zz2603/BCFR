...
## import dataset "bc_fu.csv"
bc_fu <- read_csv("C:/Users/zzhao/Desktop/BCFR/data/bc_fu.csv") %>% 
  janitor::clean_names()

## import dataset "bc_fu2.csv"
bc_fu2 <- read_csv("C:/Users/zzhao/Desktop/BCFR/data/bc_fu2.csv") %>% 
  janitor::clean_names()

## import dataset "bc_fu3.csv"
bc_fu3 <- read_csv("C:/Users/zzhao/Desktop/BCFR/data/bc_fu3.csv") %>% 
  janitor::clean_names()
...
## Exclude patient who were diagnosed with breast cancer or had mastectomy of both breasts before the date of interview.
...
There are 5626 subjects who answered 'yes', 64 subjects answered 'no', and 8452 subjects with N/A. We cannot exclude those with N/A directly. We still need to check their date or age at diagnosis. Also, there could be recall bias happened. We need to check date of diagnosis for all of them.
...
# see the distribution of breast cancer site
cancer %>% group_by(site) %>% 
  summarize(freq = n()) %>% 
  filter(site %in% c("C500","C501","C502","C503",
                     "C504","C505","C506","C507",
                     "C508","C509"))

```

Based on ICD-O-3, we only choose the sites: C500-509. Also, for ICD-O-3 5th digit behavior code, we only select '2' (Carcinoma in situ) and '3' (Malignant). 
...
There are `r length(dx_NA_id)` rows with missing dxdate and agedx. There isn't any rows with missing dt_interview and age_interview.
...
There are 5 subjects with missing agedx. We can compare the dxdate alternatively. We next see how many subjects have equal agedx and age_interview. 
...
There are 312 subjects with equal agedx and age at interview. For those subjects, we also need to compare the dxdate and dt_interview alternatively.
...
Among 317 subjects with missing agedx or equal agedx and age_interview, there are 19 subjects with daignosed date after interview date (+2 months), 284 subjects with diagnosis date before or equal to interview date (+2 months), and 14 subjects with NA. The NA is caused by missing value in month and day.
...
There are 6 subjects with equal year of dx and interview and 8 subjects with unequal year of dx and interview.
...
There is no subject with smaller interview year; there are 8 subjects with greater interview year. 
...
There are 7173 subjects left in the dataset.

### mastectomy

We can check answers to the question of mastectomy, but it didn't show if both breasts are completely removed. We need to looked at variables 'br_r_rem', 'br_l_rem', 'br_r_rem_age', and 'br_l_rem_age'. If subjects' both breasts are removed before the date of interview date, they should be removed from the dataset.

```{r echo=FALSE}
# view the distribution of subjects who answered question of mastectomy
bc_data7 %>% select(new_id,dt_interview,age_interview,
                    br_r_rem,br_r_rem_age,
                    br_l_rem,br_l_rem_age) %>% 
  group_by(br_r_rem,br_l_rem) %>% 
  summarize(freq = n())

```

* '1' denoted as yes; '2' denoted as no; '3' denoted unknown; '9' denoted missing,

* there are 70 subjects with both breast removed. We need check their ages.

* there is 1 subject with unknown laterality. We need to check her age of mastectomy and then make decision. If her age of mastectomy is missing, we need to remove this subject.

* there are 73 subjects with missing on both breast. They should be excluded.

```{r echo=FALSE}
# create a new tibble for variables related to mastectomy
bc_mas <- bc_data7 %>% 
  select(new_id,dt_interview,age_interview,
         br_r_rem,br_r_rem_age,
         br_l_rem,br_l_rem_age)

# check the subject with unknown laterality
bc_mas %>% filter(br_l_rem == 3)
# since their age of mastectomy are missing as well, we have to exclude it
bc_mas <- bc_mas %>% filter(br_r_rem!=3 | br_l_rem!=3)

# remove subjects with missing info on variables 'br_r_rem' and 'br_l_rem'
bc_mas <- bc_mas %>% filter(br_r_rem!=9 | br_l_rem!=9)

# check subjects with both breast removed
# check for missing age of mastectomy
bc_mas %>% filter(br_r_rem==1 & br_l_rem==1) %>% 
  filter(br_r_rem_age==999 | br_l_rem_age==999)

```

There are 11 subjects with both breast removed but missed age of mastectomy. They should be removed.

We want to check if there are subjects whose age of mastectomy is greater than age of interview.

```{r echo=FALSE}
# see those whose age of mastecomy are greater than age of interview
bc_mas %>% filter(br_r_rem==1 & br_l_rem==1) %>% 
  filter(br_r_rem_age>age_interview | br_l_rem_age>age_interview)

# see those whose age of mastecomy are smaller than or equal to age of interview
bc_mas %>% filter(br_r_rem==1 & br_l_rem==1) %>% 
  filter(br_r_rem_age<=age_interview & br_l_rem_age<=age_interview)

```

There are no subjects with both breasts removed whose age at mastectomy is greater than age of interview.

Other subjects' age at mastectomy are smaller than or equal to age of interview. 

All in all, 70 subjects with both breast removed should be removed. In total, we removed 143 subjects with missing info on mastectomy and age of matectomy.

```{r include=FALSE}
# exclude those with both breast removed
bc_mas <- bc_mas %>% 
  filter(br_r_rem!=1 | br_l_rem!=1)

# extract id
bc_mas_id <- bc_mas %>% pull(new_id)

# create a new tibble after filtering for mastectomy
bc_data8 <- bc_data7 %>% 
  filter(new_id %in% bc_mas_id)

```

There are 7029 subjects left in the dataset.

## Check living status

We want to check the living status of subjects. 

```{r echo=FALSE}
# create a new tibble including all variable related to living status
bc_vs <- bc_data8 %>% 
  select(new_id,dt_interview,age_interview,vs,
         livedate,age_death,age_death_est,dthdate,dth_cause)

# see distribution of living status
bc_vs %>% group_by(vs) %>% 
  summarize(freq = n())

```

There are 6369 alive subjects and 660 dead subjects.

```{r echo=FALSE}
# see how many subjects who are alive but miss the info about last known living date
bc_vs %>% filter(vs==1) %>% 
  filter(livedate==99999999)

# see how many subjects who are alive but have NA about living date
bc_vs %>% filter(vs==1) %>% 
  filter(is.na(livedate))

# see how many subjects wbo are dead but miss the info about age of death and date of death
bc_vs %>% filter(vs==2) %>% 
  filter(age_death==999 & dthdate==99999999)

# create new id variable that indicate id of dead subjects with missing info in both age of death and date of death
bc_vs_dth_miss_id <- bc_vs %>% filter(vs==2) %>% 
  filter(age_death==999 & dthdate==99999999) %>% 
  pull(new_id)

bc_vs_dth_miss_id <- c(bc_vs_dth_miss_id,14046)
bc_vs_id <- bc_vs %>% pull(new_id)

id_loc <- rep(0,length(bc_vs_dth_miss_id)) # create a vector to show the location of id
for (i in 1:length(bc_vs_dth_miss_id)) {
  id_loc[i] <- which(bc_vs_id==bc_vs_dth_miss_id[i])
}

bc_vs_id <- bc_vs_id[-id_loc]

bc_data8 <- bc_data8 %>% filter(new_id %in% bc_vs_id)

```

* There are 12 alive subjects with missing info in livedate. We cannot exclude them because they are not dead and may still be followed.

* There are 287 alive subjects with N/A in livedate. We cannot exclude them because they are not dead and may still be followed.

* There are 54 dead subjects with missing info in both age of death and date of death.They should be excluded. 

There are 6974 subjects left in the dataset.

# Calculating follow-up and diet score. 

## Follow-up 

After finishing exclusion at the beginning, we are going to calculate follow-up year.

We need to merge the cleaned data with other follow-up datasets.

```{r include=FALSE}
# merge the bc_data8 with bc_fu
bc_data9 <- left_join(bc_data8, bc_fu, by="new_id") %>% 
  select(-center_no.y)
colnames(bc_data9)[2] <- "center_no"

bc_data10 <- left_join(bc_data9,bc_fu2,by="new_id") %>% 
  select(-center_no.y)
colnames(bc_data10)[2] <- "center_no"

bc_data11 <- left_join(bc_data10,bc_fu3,by="new_id") %>% 
  select(-center_no.y)
colnames(bc_data11)[2] <- "center_no"

```

### case

Extract all subjects with diagnosis date or age.

```{r include=FALSE}
# create a subset of tibble bc_data11 for those subjects with dx info
bc_data11_case <- bc_data11 %>% filter(!is.na(agedx))

# select variables related with follow-up year calculation
# also include variables related to mastectomy
bc_case <- bc_data11_case %>% select(new_id,dt_interview,age_interview,
                          dob,dxdate,agedx,vs,age_death,dthdate,
                          br_r_rem,br_l_rem,br_r_rem_age,br_l_rem_age,
                          dt_interview_fu,age_interview_fu,
                          br_r_rem_fu,br_l_rem_fu,
                          br_r_rem_age_fu,br_l_rem_age_fu,
                          dt_interview_fu2_p,age_interview_fu2_p,
                          br_r_rem_age_fu2_p,br_l_rem_age_fu2_p,
                          dt_interview_fu3,br_r_rem_age_fu3,br_l_rem_age_fu3)
                          
                        
```

There are 473 cases in total. 

Before we do the calculation, we need to extract those with missing age of diagnosis or equal age of diagnosis and interview.

```{r echo=FALSE}
# see how many subjects with missing agedx.
bc_case %>% filter(agedx==999)

# see how many subjects with equal age of dx and interview
bc_case %>% filter(agedx==age_interview)

```

There are 2 subjects with with missing age of diagnosis; there are 18 subjects with equal age of diagnosis and interview.

Set age of interview as start age and age of diagnosis as end age. Calculate the follow-up years using end age substracted by start age. If the end age is missing, we use date to calculate the end age.

```{r}
bc_case <- bc_case %>% mutate(start_age = age_interview,
                              end_age = agedx)

miss_id <- bc_case %>% filter(agedx==999) %>% pull(new_id)
bc_case_id <- bc_case %>% pull(new_id)

bc_case[which(bc_case_id %in% miss_id)[1],28] <- as.period(ymd(bc_case[which(bc_case_id %in% miss_id)[1],5]) - ymd(bc_case[which(bc_case_id %in% miss_id)[1],4])) / years(1)

bc_case[which(bc_case_id %in% miss_id)[2],28] <- as.period(ymd(bc_case[which(bc_case_id %in% miss_id)[2],5]) - ymd(bc_case[which(bc_case_id %in% miss_id)[2],4])) / years(1)

bc_case <- bc_case %>% mutate(follow_up = end_age - start_age)

bc_case %>% filter(follow_up==0)

```

There are 18 subject with 0 follow-up years because their follow-up is less than 1 year, which means their age of interview and diagnosis is equal.

We don't want to miss those subjects, so we re-calculate the start age and end age with date. Then, we can get their follow-up.

```{r}
# create variable for id of subjects with 0 fu and incomplete dxdate
case_fu0_id <- bc_case %>% filter(follow_up==0) %>% 
  filter(!is.na(ymd(dxdate))) %>% 
  pull(new_id)
# re-calculate the start age and end age with date
for (i in 1:length(case_fu0_id)) {
  bc_case[which(bc_case_id %in% case_fu0_id)[i],27] <-
    as.period(ymd(bc_case[which(bc_case_id %in% case_fu0_id)[i],2]) -
                ymd(bc_case[which(bc_case_id %in% case_fu0_id)[i],4])) / years(1)
  bc_case[which(bc_case_id %in% case_fu0_id)[i],28] <-
    as.period(ymd(bc_case[which(bc_case_id %in% case_fu0_id)[i],5]) -
                ymd(bc_case[which(bc_case_id %in% case_fu0_id)[i],4])) / years(1)
  bc_case[which(bc_case_id %in% case_fu0_id)[i],29] <-
    bc_case[which(bc_case_id %in% case_fu0_id)[i],28] - 
    bc_case[which(bc_case_id %in% case_fu0_id)[i],27]
} 

# create variable for id of subjects with 0 fu
case_fu0_id <- bc_case %>% filter(follow_up==0) %>% pull(new_id)
# calculate and fill the fu mannually
bc_case[which(bc_case_id %in% case_fu0_id)[1],29] <- 0.67
bc_case[which(bc_case_id %in% case_fu0_id)[2],29] <- 0.33

summary(bc_case$follow_up)
...
Mode(bc_case$follow_up)

```

The mean follow-up of case is 7.64 years. The maximum follow-up year is 21 years; the minimum follow-up year is 0.23 years (less than 1).

### control

The censored subject include: death, lost to follow-up, mastectomy with both breast removed.

The control subjects are those with NA in dxdate and agedx.

```{r}
# create a new tibble for control
bc_data11_ctrl <- bc_data11 %>% filter(is.na(agedx))

# select variables related to follow-up
bc_ctrl <- bc_data11_ctrl %>% select(new_id,dt_interview,age_interview,
                          dob,dxdate,agedx,vs,age_death,dthdate,
                          br_r_rem,br_l_rem,br_r_rem_age,br_l_rem_age,
                          dt_interview_fu,age_interview_fu,
                          br_r_rem_fu,br_l_rem_fu,
...
We want to separate bc_ctrl into three parts: death, mastectomy with both breasts removed, and others (complete study or lost to follow-up).

We may check the mastectomy first because subjects with two breasts removed will be considered as censored even if they died after the mastectomy.

```{r}
# create a new tibble with alive subjects with mastectomy of both breasts
# the tibble will be separated by 1 baseline and 3 fu questionnaires.

# see how many subjects have both breasts removed at baseline
bc_ctrl %>% filter(br_r_rem_age!=999&br_l_rem_age!=999)
# at baseline questionnaire, no one has both breast removed.

# see how many subjects have both breasts removed at first follow-up
bc_ctrl %>% filter(br_r_rem_age_fu!=999&br_l_rem_age_fu!=999)
# there are 61 subjects with both removed breasts
# create a new tibble for those 61 subjects
bc_ctrl_mas_fu <- bc_ctrl %>% filter(br_r_rem_age_fu!=999&br_l_rem_age_fu!=999)

# see how many subjects have both breasts removed at second follow-up
bc_ctrl %>% filter(br_l_rem_age_fu2_p!=999&br_r_rem_age_fu2_p!=999) 
# there are 41 subjects with both breast removed
# create a new tibble for those 41 subjects
bc_ctrl_mas_fu2 <- bc_ctrl %>% filter(br_l_rem_age_fu2_p!=999&br_r_rem_age_fu2_p!=999) 

# see how many subjects with both breasts removed at third follow-up
bc_ctrl %>% filter(br_l_rem_age_fu3!=999&br_r_rem_age_fu3!=999)
# there are 71 subjects with both breasts removed
# create a new tibble for those 71 subjects
bc_ctrl_mas_fu3 <- bc_ctrl %>% filter(br_l_rem_age_fu3!=999&br_r_rem_age_fu3!=999)
